...
namespace Controller
{
    public class Function
    {
        ...
        {
            string banner = banner_field.ToString();
            return banner == "1" ? "<a href='/' id='logo' class='fl'>" + GetImage(Banner_type, Banner_ID, Banner_Image) + "</a>" : GetImagebanner(Banner_type, Banner_ID, Banner_Image);
        ...
        public string Bind_SocialNetwork(object Type, object Description, object Nickname)
        {
            try
            {
                ...
                {
                    case 3:
                        _sResult += "<a href='" + Utils.CStrDef(Nickname) + "' target='_blank' title='" + Utils.CStrDef(Description) + "'><span class='icon_facebook'></span></a>";
                        break;
                    case 4:
